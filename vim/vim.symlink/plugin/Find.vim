...
"
" Modified not to find svn files, and not to look in ./vendor or ./tmp/cache
" 
...
function! Find(name)
  ...
  let l:list=system("find . -iname '*".l:_name."*' -not -name \"*.class\" -and -not -name \"*.swp\" -not -name \"*.svn*\" | grep -v '^\.\/\.git' | grep -v '^\.\/vendor' | grep -v '^\.\/tmp\/cache' | grep -v '^\.\/tmp\/sass-cache' | perl -ne 'print \"$.\\t$_\"'")
